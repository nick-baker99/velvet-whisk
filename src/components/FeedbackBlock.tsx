  ...
  return (
    <article className="bg-white p-6 shadow-lg border border-[#F4F7D9] rounded-2xl hover:shadow-xl transition-all duration-300">
      <div className="flex items-center gap-1 mb-6">
        ...
        {[...Array(rating)].map((x, i) => <img key={i} className="w-5 h-5" src={starImage} alt="star" />)}
      </div>
      <p className="text-sm text-wood-700 opacity-90 mb-4.5"><i>"{comments}"</i></p>
      <hr className="text-burgundy-500 opacity-30 mb-6" />
      <p className="text-md text-wood-700 mb-1.5">{name}</p>
      <p className="text-sm text-wood-700 opacity-80">{customerType}</p>
    </article>
  )
}
